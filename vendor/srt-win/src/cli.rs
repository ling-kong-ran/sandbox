...
enum Cmd {
    ...
    Exec {
        ...
        #[arg(long)]
        quiet: bool,
        /// Aggregate memory limit for the complete child Job Object.
        #[arg(long)]
        memory_bytes: u64,
        /// Aggregate user-mode CPU time limit for the complete child Job Object.
        #[arg(long)]
        cpu_time_ms: u64,
        /// Maximum number of active processes in the child Job Object.
        #[arg(long)]
        processes: u32,
...
mod tests {
    ...
    fn exec_quiet_flag_parses() {
        ...
        with_args.extend(["--quiet", "--", "cmd.exe"]);
        let with = Cli::try_parse_from(with_args).expect("parse");
        assert!(matches!(with.cmd, Cmd::Exec { quiet: true, .. }));
        let mut without_args = args.to_vec();
        without_args.extend(["--", "cmd.exe"]);
        let without = Cli::try_parse_from(without_args).expect("parse");
