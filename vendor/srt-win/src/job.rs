...
impl Job {
    ...
    pub fn new(breakaway_ok: bool, limits: Option<&crate::runner::RunnerLimits>) -> Result<Self> {
        ...
        unsafe {
            ...
            if let Some(limits) = limits {
                if limits.memory_bytes == 0 || limits.cpu_time_ms == 0 || limits.processes == 0 {
                    return Err(anyhow::anyhow!(
                        "Job Object limits must be greater than zero"
                    ));
                }
                ext.BasicLimitInformation.LimitFlags |= JOB_OBJECT_LIMIT_JOB_MEMORY
                    | JOB_OBJECT_LIMIT_JOB_TIME
                    | JOB_OBJECT_LIMIT_ACTIVE_PROCESS;
                ext.BasicLimitInformation.ActiveProcessLimit = limits.processes;
                ext.BasicLimitInformation.PerJobUserTimeLimit = (limits
                    .cpu_time_ms
                    .saturating_mul(10_000)
                    .min(i64::MAX as u64))
                    as i64;
                ext.JobMemoryLimit = limits.memory_bytes.min(usize::MAX as u64) as usize;
            }
